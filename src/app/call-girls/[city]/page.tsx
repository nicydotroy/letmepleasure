    ...
    <div className="min-h-screen bg-[#FFF1F7]">
      ...
      <div className="hero-navy py-12 px-4 relative overflow-hidden">
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {/* Breadcrumb */}
          <nav className="flex items-center gap-2 text-xs text-pink-400 mb-5">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-5">
            <div>
              <p className="text-pink-300 text-sm mb-1.5 flex items-center gap-1.5">
                <MapPin size={13} className="text-pink-500" /> {city.state}
              </p>
              <h1 className="text-3xl sm:text-4xl font-black text-white mb-3">
                Call Girls in <span className="text-brand-gradient">{city.name}</span> | Escorts Service
              </h1>
              ...
              <div className="flex flex-wrap gap-3">
                <span className="text-xs bg-white/10 text-pink-200 px-3 py-1 rounded-full border border-white/10">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10">
        ...
        <section>
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-xl font-black text-[#2A0618] flex items-center gap-2">
              <Building2 size={18} className="text-pink-500" /> Browse by Area
            </h2>
          </div>
            ...
            {city.areas.map((area) => {
              ...
              return (
                <Link
                  ...
                  className="group bg-white border border-transparent rounded-2xl p-4 text-center hover:border-pink-200 hover:shadow-lg hover:shadow-pink-100/60 hover:-translate-y-1 transition-all duration-200"
                >
                  <div className="w-9 h-9 rounded-xl bg-pink-50 group-hover:bg-pink-100 flex items-center justify-center mx-auto mb-2.5 transition-colors">
                    <MapPin size={15} className="text-pink-500" />
                  </div>
                  <div className="text-xs font-bold text-slate-700 group-hover:text-pink-600 transition-colors leading-tight">
                    {area.name}
                  </div>
                  {stat ? (
                    <div className="text-xs text-pink-600 font-semibold mt-1">{stat._count.id} ads</div>
                  ) : (
        ...
        <section>
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-xl font-black text-[#2A0618] flex items-center gap-2">
              <Zap size={18} className="text-pink-600" /> Browse by Service
            </h2>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
            {CATEGORIES.map((category) => (
              <Link
                ...
                className={`group ${category.color} rounded-2xl p-6 text-center hover:shadow-lg hover:shadow-pink-100/60 hover:-translate-y-1 transition-all duration-200 border border-transparent hover:border-pink-200`}
              >
        ...
        <section>
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-xl font-black text-[#2A0618]">All Ads in {city.name}</h2>
          </div>
