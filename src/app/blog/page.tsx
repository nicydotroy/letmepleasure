    ...
    <div className="min-h-screen bg-[#EEF2FF]">
      <div className="bg-[#060B27] py-14 px-4">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-blue-300 text-sm">Tips, guides and updates from the Listvoo team</p>
        </div>
      </div>
          ...
          <div className="space-y-6">
            {posts.map((post) => (
              <Link
                key={post.id}
                href={`/blog/${post.slug}`}
                className="block bg-white rounded-2xl overflow-hidden shadow-sm border border-indigo-50 hover:shadow-md transition-shadow"
              >
                {post.coverImage && (
                ...
                <div className="p-6">
                  <h2 className="text-xl font-black text-[#060B27] mb-2">{post.title}</h2>
