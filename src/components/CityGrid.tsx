    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
      {CITIES.map((city) => (
        <Link
          ...
          className="group relative bg-white rounded-2xl p-5 border-2 border-transparent shadow-sm hover:border-pink-200 hover:shadow-xl hover:-translate-y-1.5 transition-all duration-200 text-center overflow-hidden"
        >
          {/* Background glow */}
          ...
          <div className="relative">
            ...
            <h3 className="font-black text-sm text-[#2A0618] group-hover:text-pink-700 transition-colors leading-tight">
              {city.name}
            </h3>
            <p className="text-[11px] text-slate-400 mt-0.5">{city.state}</p>
            <div className="flex items-center justify-center gap-1 mt-2 text-[11px] text-pink-400 font-semibold">
