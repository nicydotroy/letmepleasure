    ...
    <form onSubmit={handleSearch} className="w-full max-w-3xl mx-auto">
      ...
      <div className={`flex flex-col sm:flex-row bg-white ${large ? 'rounded-2xl p-2 shadow-2xl shadow-black/30' : 'rounded-xl p-1.5 shadow-lg'}`}>
        ...
        <div className="relative flex-1">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-indigo-300 pointer-events-none" size={18} />
          <input
            type="search"
        ...
        <button
          type="submit"
          className={`flex items-center justify-center gap-2 bg-[#060B27] hover:bg-[#0B1354] text-white ${large ? 'px-8 py-3.5 text-base' : 'px-5 py-2.5 text-sm'} rounded-xl font-bold transition-all shrink-0 shadow-lg`}
        >
          <Search size={16} />
          Search
        </button>
