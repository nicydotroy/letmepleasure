...
export default function CustomLocationContent({ content }: Props) {
  ...
  return (
    <section className="bg-white rounded-3xl p-8 sm:p-10 border border-indigo-50 shadow-sm">
      ...
      <div className="max-w-4xl mx-auto">
        {content.heading && (
          <h2 className="text-2xl sm:text-3xl font-black text-[#060B27] mb-4">{content.heading}</h2>
        )}
        {content.intro && (
        ...
        {content.faqs.length > 0 && (
          <div className="mt-10">
            <h3 className="text-xl font-black text-[#060B27] mb-5">Frequently Asked Questions</h3>
            <div className="space-y-3">
              {content.faqs.map((f, i) => (
                <details
                  key={i}
                  className="group bg-[#EEF2FF] rounded-xl border border-indigo-100 p-4 open:shadow-sm transition-shadow"
                >
                  <summary className="flex items-center justify-between gap-3 cursor-pointer list-none font-bold text-[#060B27] text-sm sm:text-base">
                    {f.q}
                    <span className="shrink-0 w-6 h-6 rounded-full bg-indigo-100 text-indigo-700 flex items-center justify-center text-lg leading-none transition-transform group-open:rotate-45">
                      +
                    </span>
                  </summary>
